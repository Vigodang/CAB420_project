...
def run_inference_on_file(imagefile, predsfile, model, size=SIZE, use_elevation=True, elevafile=None):
    ...
    batch_size = 64
    for i in tqdm(range(0, len(chips), batch_size), desc='Inferring', unit='batch'):
        batch = chips[i:i+batch_size]
        
        imgs = np.array([chip[..., :3] for chip, _, _ in batch])
        elvs = np.array([chip[..., 3:] for chip, _, _ in batch])
        
        preds = model.predict([imgs, elvs], verbose=0)
        
        for (chip, x, y), pred in zip(batch, preds):
